...
### Persónuverndarstefna / upplýst samþykki (0034)
Samþykkis-hakið í skráningu (t.d. „Ég samþykki að upplýsingar mínar séu unnar vegna viðburðarins") fær nú hlekk **„Lesa persónuverndarstefnu"** sem opnar opinbera síðu `/{orgSlug}/personuvernd`. Stefnan er geymd per fyrirtæki (`organizations.privacy_policy`) og ritstýrð í **Persónuvernd-stjórnborðinu** (ritill með GDPR-sniðmáti). Tóm stefna → síðan sýnir hlutlausan fallback. Vistun fer um admin-client, gætt við starfsfólk. Krefst `0034_privacy_policy.sql`.

> Sniðmátið er drög með hornklofa-staðgenglum ([nafn], [netfang], [varðveislutími]) — yfirfara og helst lögfræðilega skoðun áður en birt.


### Google Wallet (sofandi þar til skilríki eru sett)
