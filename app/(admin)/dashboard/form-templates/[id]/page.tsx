...
import { getOrCreateTemplateEvent } from "../actions";

export default async function FormTemplateEditPage({ params }: { params: { id: string } }) {
  const res = await getOrCreateTemplateEvent(params.id);
  if (!res.ok || !res.eventId) {
    if (res.error === "Sniðmát fannst ekki.") notFound();
    return (
      <div className="max-w-3xl space-y-4">
        <Eyebrow>Form sniðmát</Eyebrow>
        <PageTitle>Form</PageTitle>
        <Card accent>
          <p className="text-sm text-danger">{res.error ?? "Tókst ekki að opna form."}</p>
        </Card>
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        <Eyebrow>Form sniðmát</Eyebrow>
        <PageTitle>{res.label ?? "Form"}</PageTitle>
        <p className="mt-2 text-sm text-muted">
          Veldu reitina í þessu formi. Nýir viðburðir sem nota það fá afrit. Breytingar hér hafa ekki áhrif á viðburði
          sem þegar eru til.
        </p>
        <p className="mt-1">
